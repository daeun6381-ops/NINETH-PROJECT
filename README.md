...
<head>
    ...
    <title>Mate Diary - 우리들의 기록</title>
...
<body id="main-body" class="bg-slate-50 text-slate-800">
    ...
    <div id="connection-screen" class="fixed inset-0 bg-slate-100 z-[100] flex items-center justify-center p-4">
        <div class="bg-white p-8 rounded-3xl shadow-2xl max-w-sm w-full text-center border border-slate-200">
            <h2 class="text-3xl font-bold text-teal-600 mb-6 font-handwriting">우리들의 일기장</h2>
            
            <div id="setup-step-1">
                <p class="text-slate-500 mb-8 text-sm leading-relaxed">소중한 사람과 연결하여<br>실시간으로 일기를 공유해보세요.</p>
                <button onclick="showStep('join')" class="w-full bg-teal-500 text-white font-bold py-4 rounded-2xl mb-3 hover:bg-teal-600 transition shadow-lg shadow-teal-100">코드 입력하고 시작하기</button>
                <button onclick="handleCreateCode()" class="w-full bg-white border-2 border-teal-500 text-teal-500 font-bold py-4 rounded-2xl hover:bg-teal-50 transition">새로운 연결 코드 만들기</button>
            </div>

            <div id="setup-step-join" class="hidden">
                <p class="text-sm text-slate-500 mb-4 text-center font-bold text-teal-600">입장 코드 입력</p>
                <input id="join-code-input" type="text" placeholder="6자리 코드" maxlength="6" class="w-full p-4 rounded-2xl bg-slate-50 mb-4 border-2 border-teal-100 focus:border-teal-400 outline-none text-center font-bold tracking-widest uppercase text-xl">
                <button onclick="handleJoin()" class="w-full bg-teal-600 text-white font-bold py-4 rounded-2xl mb-3">연결하기</button>
                <button onclick="showStep('1')" class="text-slate-400 text-sm underline cursor-pointer">뒤로가기</button>
            </div>

            <div id="setup-step-wait" class="hidden">
                <p class="text-sm text-slate-500 mb-3">상대방에게 아래 코드를 전달하세요</p>
                ...
                <div class="flex flex-col items-center gap-3">
                    ...
                    <p class="text-xs text-teal-500 animate-pulse">상대방이 코드를 입력하길 기다리고 있어요...</p>
                    <button onclick="window.location.reload()" class="mt-4 text-slate-400 text-sm underline cursor-pointer">취소하고 다시하기</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <header class="bg-white/70 backdrop-blur-md py-8 px-4 text-center border-b border-white/20">
        <div class="max-w-4xl mx-auto relative">
            ...
            <h1 id="d-day-display" class="text-4xl font-bold text-teal-600 font-handwriting">함께한 지 1일</h1>
            <p class="text-xs text-slate-400 mt-2 font-bold tracking-widest"><span id="nick-me">나</span> ❤️ <span id="nick-partner">상대방</span></p>
        </div>
    </header>

    <main class="max-w-4xl mx-auto p-6">
        <section class="bg-white rounded-3xl p-6 shadow-sm mb-10 border border-slate-100">
            <input id="diary-title" type="text" placeholder="오늘의 제목" class="w-full p-4 mb-4 rounded-xl bg-slate-50 border-none outline-none focus:ring-2 ring-teal-100 font-bold">
            <textarea id="diary-content" rows="4" placeholder="오늘 어떤 소중한 일이 있었나요?" class="w-full p-4 mb-4 rounded-xl bg-slate-50 border-none outline-none focus:ring-2 ring-teal-100 resize-none"></textarea>
            ...
            <div class="flex items-center justify-between gap-4">
                <div class="flex items-center gap-3">
                    <label class="cursor-pointer bg-slate-100 p-3 rounded-xl hover:bg-slate-200 transition border border-slate-200" title="사진 추가">
                    ...
                    <select id="diary-mood" class="bg-slate-100 p-3 rounded-xl text-sm outline-none border border-slate-200">
                        <option value="😊">😊 평온</option><option value="✨">✨ 기쁨</option><option value="🥱">🥱 피곤</option><option value="🍕">🍕 맛남</option><option value="☁️">☁️ 우울</option>
                    </select>
                </div>
                <button onclick="handleSaveDiary()" id="btn-save" class="bg-teal-500 text-white font-bold py-3 px-8 rounded-xl shadow-lg hover:bg-teal-600 transition flex-1 sm:flex-none">기록하기</button>
            </div>
        </section>

        <div id="diary-list" class="grid grid-cols-1 md:grid-cols-2 gap-6 pb-20">
            <div class="col-span-full text-center py-20 text-slate-400">일기를 불러오는 중입니다...</div>
        </div>
    </main>
    ...
    <div id="settings-modal" class="modal">
        <div class="bg-white w-full max-w-md rounded-3xl p-6 shadow-2xl mx-4 border border-slate-200">
            <div class="flex justify-between items-center mb-6">
                <h3 class="font-bold text-xl">공간 관리</h3>
            ...
            <div class="space-y-4">
                <div>
                    <label class="block text-xs font-bold text-slate-400 mb-2 uppercase">우리의 시작일</label>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-xs font-bold text-slate-400 mb-2 uppercase">나의 이름</label>
                        <input id="set-my-nick" type="text" maxlength="6" class="w-full p-4 bg-slate-50 rounded-xl outline-none border border-slate-100">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-400 mb-2 uppercase">너의 이름</label>
                ...
                <button onclick="handleSaveSettings()" class="w-full bg-teal-600 text-white font-bold py-4 rounded-2xl shadow-md hover:bg-teal-700 transition mt-2">저장하기</button>
                <button onclick="handleLogout()" class="w-full text-slate-300 text-[10px] underline pt-4 uppercase tracking-tighter">연결 해제 및 초기화</button>
            </div>
        </div>
    </div>
    ...
    <script type="module">
        ...
        window.handleJoin = async () => {
            ...
            try {
                ...
                if(snap.exists() && snap.data().status === 'waiting') {
                    ...
                    alert('유효하지 않거나 이미 만료된 코드입니다.');
                }
            } catch(e) { alert('연결 중 오류가 발생했습니다.'); }
        };
        ...
        window.handleSaveDiary = async () => {
            ...
            try {
                ...
                window.removeImage();
            } catch (e) { alert('기록 저장에 실패했습니다.'); }
            finally { btn.disabled = false; btn.innerText = "기록하기"; }
        };
        ...
        window.handleSaveSettings = async () => {
            ...
            const updates = { startDate, [`nicks.${user.uid}`]: myNick || '나' };
            if(partnerId) updates[`nicks.${partnerId}`] = partnerNick || '너';
        ...
        window.handleLogout = () => {
            if(confirm('연결을 해제하시겠습니까? (저장된 일기는 삭제되지 않지만 다시 코드로 입장해야 합니다)')) {
        ...
        const renderDiaries = (col, lastWritten, partnerId) => {
            onSnapshot(col, (snap) => {
                ...
                if(items.length === 0) {
                    list.innerHTML = '<div class="col-span-full text-center py-20 text-slate-300">함께 써 내려갈 첫 페이지를 기다려요.</div>';
                    return;
                }
                ...
                items.forEach(data => {
                    ...
                    if(isLocked) {
                        div.innerHTML = `<div class="flex items-center justify-center h-48 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200"><p class="text-sm text-slate-400 font-bold">🔒 오늘 일기를 쓰면 열려요!</p></div>`;
                    } else {
